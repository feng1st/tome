...
- display 侧新增 位置→Transform 同步（`frontend/display/movement`）：格坐标 ×TILE_SIZE、Y 翻转；朝向判断（`sprite_animation` 的 `sync_animation`）改用格坐标位移。
...
- `core/map` 不再含像素概念；`frontend/display/` 新增位置同步（`movement`）、形象注册表（`appearance`）与双动画域（`sprite_animation`/`terrain_animation`）；`frontend/input/` 持有手势与分发。
